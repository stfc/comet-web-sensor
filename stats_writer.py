...
class StatsWriter():
    def __init__(self, config_file='config.ini', update_interval=7200):
        ...
        self._measurement_files = { 'Temperature':'Temp_stats.csv',
    ...
    def _calc_stats(self, column, dataframe):
        stats = (
                dataframe.groupby(['date','ip','name'], as_index=False).
                agg({column:[np.max, np.mean, lambda d: np.sqrt(np.mean(np.square(d)))]})
                )
        return stats
